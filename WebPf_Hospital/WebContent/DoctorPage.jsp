...
<body>
	<jsp:include page="Header.jsp"/>
		...
		<tr><th>번호</th><th>진료시간</th><th>담당의사</th><th>특이사항</th></tr>
		<%
		...
		try{
			...
			ResultSet rs = stmt.executeQuery("select * from 진료확인 where 회원id='"+id+"';");
			
			int i=1;
			while(rs.next()){
				String time = rs.getString("진료시간");
				out.print("<tr><td>"+i+"</td>");
				out.print("<td>"+time.substring(0, time.length()-5)+"</td>");
				out.print("<td>"+rs.getString("이름")+"</td>");	
				out.print("<td>"+rs.getString("특이사항")+"</td></tr>");
				i++;
			}
		}catch(Exception e){
			e.getMessage();
